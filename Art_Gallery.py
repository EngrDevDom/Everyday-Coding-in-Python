...
def addartist():
    ...
    cursor.execute(""" INSERT INTO Artists (name,address,town,country,postcode)
...
def viewartists():
    ...
    for x in cursor.fetchall():
        newrecord = str(x[0]) + ", " + str(x[1]) + ", " + str2(x[2]) + ", " + str2(x[3]) + ", " + str2(x[4]) + ", " + x[5] + "\n"
...
def searchmediumoutput():
    selectmedium = medium2.get()
    cursor.execute(""" SELECT Art.pieceid, Artists.name, Art.title, Art.medium, Art.price
    FROM Artists, Art WHERE Artists.artistsid=Art.artistid AND Art.medium=? """, [selectmedium])
    for x in cursor.fetchall():
        newrecord = str(x[0]) + ", " + str(x[1]) + ", " + str2(x[2]) + ", " + str2(x[3]) + ", " + str2(x[4]) + ", " + x[5] + "\n"
        outputwindow.insert(END,newrecord)
    medium2.set("")
...
title1.place(x = 10, y = 10, width = 80, height = 25)
...
artistcountylbl = Entry(text = "")
artistcountylbl.place(x = 850, y = 40, width = 100, height = 25)
...
artmedium = OptionMenu(window, medium, "Oil", "Ink", "Acrylic", "")
...
viewallart = Button(text = "View All Artists", command = viewart)
...
maxlbl.place(x = 1020, y = 360, width = 75, height = 25)
