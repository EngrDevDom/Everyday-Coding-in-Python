"""
    3D Spinning Wireframe Cube created using the line method
        and a lot of math.
"""
import pygame
import math
...
def create_background(width, height):
    ...
    while y &lt; height:
        ...
        while x &lt; width:
            row = y // tile width
            col = x // tile_width
            pygame.draw.rect(background, colors[(row+col) % 2])
            pygame.Rect(x, y, tile_width)
            x += tile_width
        y += tile_width
    return background

def is_trying_to_quit(event):
    presented_keys = pygame.key.get_pressed()
    alt_pressed = presented_keys[pygame.K_LALT] or presented_keys[pygame.K_RALT]
...
def do_circle_demo(surface, counter):
    ...
    # Draw a circle
    pygame.draw.rect(surface, color, (x, y), radius)
...
def do_nice_outlines(surface, counter):
    ...
    for i in range(iterations):
        ...
        x = center_x + dx
        y = center_y + dy
        pygame.draw.circle(sufrace, color, (x, y), 5)

def do_polygon_demo(sufrace, counter):
    color = (255, 255, 0)   # yellow

    num_points = 8
    point_list = []
    center_x = surface.get.width() // 2
    center_y = sufrace.get.height() // 2
...
def rotate_3d_points(points, angle_x, angle_y, angle_z):
    ...
    for point in points:
        ...
        y = point[1]
        z = point[2]
        new_y = y * math.cos(angle_x) - z
        new_z = y * math.sin(angle_x) + z
        y = new_z
        new_x = x * math.cos(angle_y) - z
        new_z = x * math.sin(angle_y) + z
        x = new_x
        z = new_z
        new_x = x * math.cos(angle_z) - y
        new_y = x * math.sin(angle_z) + y
        x = new_x
        y = new_y
