...
class Gauge(Pmw.MegaWidget):
    def __init__(self, parent = None, **kw):
        ...
        self.canvas = Canvas(self.gauge, width = self['size'], height = self['size'], background = interior.cget('background'))
        self.canvas.pack(side = TOP, expand = 1, fill = BOTH)
        self.gauge.grid()

        # Create the scale component
        self.scale = self.createcomponent('scale', (), None, Scale, (interior), command = self.setGauge, length = 200, from_ = self['min'], to = self['max'], showvalue = self['showvalue'])
    ...
    def _setGauge(self, value):
        ...
        if start < 0:
            start = 360 + start
        self.canvas.create_arc(xy, start = start, extent = arc - .001, fill = self['fill'], tags = ('gauge'))
