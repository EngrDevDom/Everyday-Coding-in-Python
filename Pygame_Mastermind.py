...
class Mastermind:
    ...
    def draw_board(self, event=None):
        self.canvas.destroy()
        self.status.destroy()
        self.canvas = tk.Canvas(self.parent, width=800, height=1000)
        self.canvas.configure(background="pink")
        self.canvas.pack()
        self.bag() = {'r':self.canvas.create_oval(0, 960, 66, 1000, fill='red', outline='red'),
                      'o':self.canvas.create_oval(66, 960, 131, 1000, fill='orange', outline='orange'),
                      'y':self.canvas.create_oval(131, 960, 200, 1000, fill='yellow', outline='yellow'),
                      'g':self.canvas.create_oval(200, 960, 267, 1000, fill='green', outline='green'),
                      'b':self.canvas.create_oval(267, 960, 334, 1000, fill='blue', outline='blue'),
                      'p':self.canvas.create_oval(334, 960, 400, 1000, fill='purple', outline='purple')
                      }
    ...
    def check(self, event=None):
        ...
        if exact == 4:
            ...
            self.status.config(text='Out of guesses. The correct answer is {}.'.format(''.join(self.pattern)))
            self.canvas.unbind('<1>')
        else:
            self.guesses.append('')
